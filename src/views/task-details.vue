...
export default {
  ...
  computed: {
      ...
      if (showActivity === true) {
        return this.currBoard.activities.filter((activity) => {
          return (
            ...
            !activity.title.includes("posted a comment to ") &&
            activity.task &&
            !activity.title.includes("replied to a comment")
          );
        });
      } else {
        return this.currBoard.activities.filter((activity) => {
    ...
    setCurrPos() {
      if (this.currAction.type === "taskCanvas")
        return {
          position: "fixed",
          top: "50%",
          left: "50%",
  ...
  methods: {
    ...
    setCommentToReply(commentId) {
      ...
      this.commentToReply = this.currBoard.activities.find(
    ...
    async saveActivity(activityTitle, isComment = false) {
      try {
        ...
        if (this.currBoard.activities.length > 50)
          this.currBoard.activities.splice(
            this.currBoard.activities.length - 1,
            1
          );
        if (isComment) {
    ...
    async updateBoard(board) {
      try {
        ...
        utilService.showErrorMsg("Sorry, Could not update the board.");
      }
    },
    closeModal() {
    ...
    async removeTask() {
      try {
        ...
        this.$router.push("../");
        utilService.showSuccessMsg("Task removed successfully");
        this.saveActivity(
          `removed the task "${oldTask.title}" from "${group.title}"`
        );
      } catch (err) {
        utilService.showErrorMsg("Sorry, Could not delete the task.");
      }
    },
    ...
    editImg(imgToEdit) {
      ...
      utilService.showSuccessMsg("Attachment edited successfully");
    },
    removeImg(img) {
      const taskToEdit = JSON.parse(JSON.stringify(this.currTask));
      const foundIdx = taskToEdit.imgs.findIndex(
        (currImg) => currImg.id === img.id
      );
      if (foundIdx < 0) return console.log("couldnt find idx");
      ...
      this.updateTask(taskToEdit);
      utilService.showSuccessMsg("Attachment removed successfully");
    },
